...
There is a few [differences from Ubuntu](https://pop.system76.com/docs/difference-between-pop-ubuntu/), but most importantly Pop!_OS provides a special image specifically for NVIDIA machines so you don't have to follow a bunch of post install steps to get thing working. 
...
3. Boot XPS 15 and hit F2 as it boots to enter the BIOS menu
4. Enter BIOS:
    ...
    - SATA Operation > AHCI (you need to turn "RAID" mode off because it is not supported by Linux, because its not worth while...)
5. Save and exit BIOS
6. Plug in your Pop!_OS USB
...
13. Checkout your new system! Check the [intro docs](https://support.system76.com/articles/pop-basics/) and [keyboard shortcuts](https://pop.system76.com/docs/keyboard-shortcuts/) for more info.
...
The "update manager" is gone, instead updates are managed from the Pop!_Shop software center. 
...
Then the next time you need a Git credential in it will prompt for username and password as normal, but it will be stored going forward.
...
Since sleep works fine on Intel graphics, which I generally use for battery life anyway, I will wait for some other brave soul to figure out the best boot options.
